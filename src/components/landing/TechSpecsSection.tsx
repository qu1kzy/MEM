  ...
  return (
    <AnimatedSection>
      <section className="py-20 px-4 max-w-6xl mx-auto" id="specs">
        ...
        <StaggerChildren className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] backdrop-blur-sm p-8 flex flex-wrap justify-between gap-8">
          {specs.map((s) => (
            <motion.div key={s.label} className="flex-1 min-w-[200px] text-center" variants={staggerItem}>
